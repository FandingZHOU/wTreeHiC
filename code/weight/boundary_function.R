ABcompartment_boundaries<-function(data_origin,bin_size){
  boundary<-c(0)
  for (i in 2:(nrow(data_origin)-1)){
...
conbined_boundaries<-function(data_origin1,data_origin2,bin_size){
  bondary1<-ABcompartment_boundaries(data_origin1,bin_size)
  bondary2<-ABcompartment_boundaries(data_origin2,bin_size)
